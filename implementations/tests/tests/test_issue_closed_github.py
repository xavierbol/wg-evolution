...
from implementations.scripts.issue_closed_github import IssueClosedGithub
...
class TestIssueClosedGithub(unittest.TestCase):
    def setUp(self):
        """
    ...
    def test_compute(self):
        """
        Test the compute method of a IssueClosedGithub
        object instantiated with default parameters.
        """
        issues = IssueClosedGithub(self.items)
        expected_count = 20
    ...
    def test_compute_reopen_as_new(self):
        """
        Test whether the reopen_as_new parameter work as expected.
        """
        issues = IssueClosedGithub(self.items, reopen_as_new=True)
        expected_count = 21
        count = issues.compute()
        self.assertEqual(expected_count, count)
